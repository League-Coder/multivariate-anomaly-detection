...
def evaluate_machine(args, machine: str) -> dict | None:
    ...
    for suffix in ["_train.npy", "_test.npy", "_test_labels.npy"]:
    ...
    test_labels = np.load(data_dir / f"{machine}_test_labels.npy")
    ...
    #diag = diagnose(test_scores, test_labels)
    #metrics.update(diag)

    # Elevation-ratio diagnosis
    #print("Computing elevation-ratio diagnosis...")
    #diag_elev = diagnose_with_elevation(test_scores, test_labels, baselines)
    ...
    if test_labels.ndim == 2:
        labels_1d = (np.sum(test_labels, axis=1) >= 1).astype(float)
    else:
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate TranAD on SWaT")

    parser.add_argument("--machine", type=str, default="swat")
    parser.add_argument("--data-dir", type=str, default="data/swat/processed")
    parser.add_argument("--model-dir", type=str, default="models/tranad/swat")
